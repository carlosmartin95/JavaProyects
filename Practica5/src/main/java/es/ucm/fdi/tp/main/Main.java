	...
	private static GameTable<?, ?> createGame(String gType) {
		...
		private static <S extends GameState<S, A>, A extends GameAction<S,A>>			
			...
			for (int i = 0; i < game.getState().getPlayerCount(); i++) {
				...
				final SmartPlayer smartPlayer = SmartPlayer("SmartPlayer");
				randomPlayer.join(i);
				smartPlayer.join(i);
				final String titulo;
				if(gType.equals("was")){
				...
				try{
					SwingUtilities.invokeAndWait(new Runnable(){
						@Override
						public void run() {
							GameController<S,A> gameCtrl = new GuiController<S,A> (randomPlayer.getPlayerNumber(), randomPlayer, smartPlayer, game);
		...
		private static SmartPlayer SmartPlayer(String string) {
			SmartPlayer smart = new SmartPlayer(string,1);
			return smart;
		}
		...
		public static <S extends GameState<S,A>, A extends GameAction<S,A>> GuiView<S, A> createGameView(String gType, GameController<S,A> gameCtrl) {
			...
			else return (GuiView<S, A>) new TttView((GameController<TttState, TttAction>) gameCtrl);
		}		
