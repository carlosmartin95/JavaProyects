...
public class WolfAndSheepState extends GameState<WolfAndSheepState, WolfAndSheepAction>{
	...
	private void comprobarLobos(int fila, int columna) {		
		...
		else if(fila==0 && columna>0 && columna<this.dim-1){
			this.finished=true;
		}
		else{
			if(columna != 0){
	...
	public boolean isWinner(int[][] board, int playerNumber) {
		...
		if(playerNumber==WOLF) {//Si es lobo miramos que haya un lobo en la fila 0
	...
	public Integer posicion(int row, int col) {
